...
import { transformEmployeeListData } from "../../utils/employeeUtils";
import authApi from "../../api/authApi";

const columns: readonly Column[] = [
    { id: 'index', label: 'Stt', minWidth: 60 },
...
export default function EmployeeList() {
    ...
    const {
        ...
        isLoading,
        data
    } = useQuery(['employee', page, rowsPerPage, searchText], async ({ signal }) => {
        if (signal && currentUser?.jwt) {
            // Use offset so page start at 0
            return await employeeApi.getAllEmployee(
                signal,
                currentUser.jwt,
                page,
                rowsPerPage,
    ...
    return (
        <Root>
            <Typography variant="h6" fontWeight="600">Nhân viên</Typography>
            <ContainerRoot>
                <Stack
                    direction='row'
                ...
                >
                    ...
                    />
                    <CustomButton buttonName="Thêm nhân viên" sx={{ width: 200 }}
                        onClick={() => navigate('add-employee')}
                    />
                </Stack>
                <StickyHeadTable
                    columns={columns}
                    data={isSuccess && transformEmployeeListData(data.data)}
                    isSuccess={isSuccess}
                    isLoading={isLoading}
                    tableKey="employee"
                    onEditClick={(id: number) => navigate(`edit-employee/${id}`)}
                    action="block"
