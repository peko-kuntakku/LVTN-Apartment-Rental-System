...
import { transformRequestListData } from "../../utils/requestUtils";
...
export default function RequestList() {
    ...
    const {
        ...
        isLoading,
        data
    } = useQuery(['requests', page, rowsPerPage, searchText], async ({ signal }) => {
        if (signal && currentUser?.jwt) {
            return await requestApi.getAllRequest(signal, currentUser.jwt, page, rowsPerPage);
        }
    }, {
        enabled: true,
    ...
    return (
        <Root>
            <Typography variant="h6" fontWeight="600">Yêu cầu dịch vụ</Typography>
            <ContainerRoot>
                <Stack
                    direction='row'
                ...
                <StickyHeadTable
                    columns={columns}
                    data={isSuccess && transformRequestListData(data.data)}
                    isSuccess={isSuccess}
                    isLoading={isLoading}
                    tableKey="request"
